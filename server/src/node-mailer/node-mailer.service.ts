...
export class NodeMailerService {
    ...
    async callbackTemplate(data: ISendData[], callback: CallbackDto) {
        const result: ResultDto = await this.mailerService
            ...
            .catch((e) => {
                console.log(e)
                return { isSuccess: false, message: 'Произошла ошибка сообщение не отправлено' + e};
            });
        return result;
    }

    // Отправка соообщения при регистрации
    async setRegister(body: CreateRegisterDto, uuid: string): Promise<ResultDto> {
        // Отправка почты
        const result: ResultDto = await this.mailerService
            .sendMail({
              to: body.email,
    ...
    // Отправка соообщения при регистрации
    async setConfirmResetPassword(email: string, uuid: string): Promise<ResultDto> {
      // Отправка почты
      const result: ResultDto = await this.mailerService
          .sendMail({
            to: email,
  ...
  async setMailFile(fileName: string, pathFile: string, date: string): Promise<ResultDto> {
    try {
