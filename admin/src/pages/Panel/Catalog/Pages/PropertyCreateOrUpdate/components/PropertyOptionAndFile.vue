<script lang="ts" setup>
    ...
    import {
        ...
        PropertyControllerClient, Result, CreateDropzoneDto
    ...
    const uploadFiles = (file: CreateDropzoneDto, item: ProdcutPropertyesOptionsDto) => {
    ...
    const changeOptions = (event: ProdcutPropertyesOptionsDto) => {
        if (event.code && event.value && !!event.path) {
            options.value.push({...nullOptions});
        }

        const filters: ProdcutPropertyesOptionsDto[] = options.value.filter((el: ProdcutPropertyesOptionsDto) => el.code && el.value && !!el.path);
        emit('change', filters);
    };
...
<template>
    <div v-if="options.length">
        ...
        >
            ...
            />
            <upload-file
                @upload-file="uploadFiles($event, item)"
                :file-prop="item.path"
            />
