<script lang="ts" setup>
    ...
    const changeOptions = (event: ProdcutPropertyesOptionsDto) => {
        if (event.code && event.value && event.value) {
            options.value.push({...nullOptions});
        }

        const filters: ProdcutPropertyesOptionsDto[] = options.value.filter((el: ProdcutPropertyesOptionsDto) => el.code && el.valueKey && el.value);
        emit('change', filters);
    };
