<script lang="ts" setup>
    import {
        FullPropertyLinksDto, getApiClientInitialParams,
        PropertyLinkControllerClient,
        PropertyLinksValuesDto, Result,
...
<template>
    <q-card class="section-create-form q-mb-lg">
        <q-card-section class="product-create__content q-pb-none">
            <div class="property-textarea-components">
                <div class="property-textarea-components__header q-mb-lg row items-center justify-between">
                    ...
                    <q-icon
                        ...
                        name="delete"
                        size="20px"
                        class="property-textarea-components__icon cursor-pointer"
                        @click="deleteProperty"
                    ></q-icon>
                </div>
                <div class="property-textarea-components__body">
                    <div
                        class="property-textarea-components__item q-mb-md row items-start justify-between no-wrap"
                        v-for="(item, index) in value"
                        :key="'editor' + index"
                    >
                        <form-text-area
                            class="property-textarea-components__item__textarea"
                            v-model="item.value"
                            :is-live-save="true"
                        ...
                        <q-icon
                            ...
                            name="delete"
                            size="20px"
                            class="property-textarea-components__icon cursor-pointer q-ml-md"
                            @click="deleteField(item, index)"
                        />
                    </div>
                    <div class="property-textarea-components__button" v-if="item.productProperty.multi">
                        <q-btn class="gradient-black" @click="addField">
                            Добавить
                        </q-btn>
                    </div>
                </div>
...
<style lang="scss" scoped>
    .product-create__content {
        &.q-pb-none {
            padding-bottom: 0;
        }
    }
    .property-textarea-components {
        &__item {
            &__textarea {
                width: 100%;
            }
