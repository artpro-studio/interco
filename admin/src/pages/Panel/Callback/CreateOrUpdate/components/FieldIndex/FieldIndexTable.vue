<script lang="ts" setup>
    ...
    import { ICallbackField, ICallbackFieldFilter, CallbackFieldColumnsTable, CallbackFieldTypeData, CallbackFieldData } from './interface';
    import { CallbackFieldControllerClient, getApiClientInitialParams, ICallbackFieldType, ICallbackFiledAttribute } from '@/ApiClient/ApiClient';

    interface IProps {
        rows: ICallbackField[];
        filters: ICallbackFieldFilter;
        count: number;
        limit: number;
    }
    ...
    const columns = CallbackFieldColumnsTable;
    ...
    <q-table :rows="rows" :columns="columns" no-data-label="Ничего не найдено" row-key="name" :rows-per-page-options="[0]">
        ...
        <template v-slot:body="props">
            <tr>
                ...
                <td class="text-left" @click="getRouterDetail(props.row.id)">
                    {{ CallbackFieldTypeData[props.row.type as ICallbackFieldType] }}
                </td>
                <td class="text-left" @click="getRouterDetail(props.row.id)">
                    {{ CallbackFieldData[props.row.attribute as ICallbackFiledAttribute] }}
                </td>
                <td class="text-left" @click="getRouterDetail(props.row.id)">
