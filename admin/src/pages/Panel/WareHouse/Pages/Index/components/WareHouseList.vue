<script lang="ts" setup>
    import { FullWarehouseDto, getApiClientInitialParams, ResponseWarehouseListDto, WareHouseControllerClient } from 'src/ApiClient/ApiClient';
    ...
    const content: Ref<FullWarehouseDto[] | null> = ref(null);
    ...
    const create = async () => {
        const result: ResponseWarehouseListDto | any = await api.get(page.value);
        if (!result.isSuccess) {
            $q.notify({
                color: 'negative',
                textColor: 'white',
...
<template>
    <div>
        ...
        <template v-else>
            <q-table :rows="content" :columns="columns" no-data-label="Ничего не найдено" row-key="name" :rows-per-page-options="[0]">
                <template v-slot:body="props">
                    <tr>
                        ...
                        <td class="text-left">
                            <router-link :to="getRouterDetail(props.row.id)">{{ props.row.timeWork }}</router-link>
                        </td>
                        <td class="text-left">
                            <router-link :to="getRouterDetail(props.row.id)">{{ props.row.phone }}</router-link>
                        </td>
                        <td class="text-left">
                ...
                <template v-slot:bottom v-if="lengthPages">
                    <pagination :page="page" :count="count" :max="lengthPages" @update="updatePage" />
                </template>
            </q-table>
        </template>
        <q-btn color="primary" class="circle fixed" :to="getRouter(RouterName.WareHouseCreate)">
