<script lang="ts" setup>
    ...
    const content: Ref<CreateOrdersDto[] | null>= ref(null);
    const limit: Ref<number> = ref(0);
    const count: Ref<number> = ref(0);
    const page: Ref<number> = ref(1);
    const api =  new OrdersControllerClient(getApiClientInitialParams());
    ...
    const create = async () => {
        const result: ResponseUsersListDto | any = await api.get(page.value);
        if (!result.isSuccess) {
            $q.notify({
                color: 'negative',
                textColor: 'white',
...
<template>
    <div>
            ...
            >
                <template v-slot:body="props">
                    <tr>
                        ...
                        <td class="text-left">
                            <router-link :to="getRouterDetail(props.row.id)">{{ props.row.firstName }}</router-link>
                        </td>
                        <td class="text-left">
                            <router-link :to="getRouterDetail(props.row.id)">{{ props.row.lastName }}</router-link>
                        </td>
                    </tr>
                </template>
        ...
        <q-btn
            color="primary"
            class="circle fixed"
            :to="getRouter(RouterName.OrdersCreate)"
        >
