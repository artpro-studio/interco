...
export class BaseQuery {
  @ApiProperty({ example: 'Поиск', description: 'Поиск' })
  search: string;

  @ApiProperty({ example: 10, description: 'Страница' })
  page: number;

  @ApiProperty({ example: 32, description: 'Количество' })
  limit: number;
}

export class BaseQueryIds {
  @ApiProperty({ type: Number, example: [1,2,3], isArray: true, description: 'ids' })
  ids: number[];
}
